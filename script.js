// отримали дані з http endpoint(витягує масив продуктів)
const getProducts = async () => {
  const response = await fetch("https://fakestoreapi.com/products?limit=9");
  const products = await response.json();
  return products;
};
//рендерить шоб ми бачили на сторінці(на екран)
...
//видаляє елемент з кошика
const removeProductFromCart = (event, product) => {
  event.target.parentElement.parentElement.remove();
  const cartListItem = document.getElementsByClassName("cart-list-item");
...
//рендерить один елемент з списку
...
// рендерить початковий вигляд кошика
...
//зберігає або оновлює в локал сторідж
...
//витягує масив елементів з локал сторідж
const getCurrentCartItems = () =>
  JSON.parse(localStorage.getItem('cart-products')) || [];

//сетає в локал сторідж(додає)
